"""Quick smoke test for the BFM-Zero training loop.

Runs a few iterations to verify the pipeline works end-to-end.
"""
import sys
import os
...
cfg = TrainConfig(
    ...
    agent=FBcprAuxAgentConfig(
        ...
        model=FBcprAuxModelConfig(
            ...
            seq_length=8,
            actor_std=0.05,
            amp=True,  # Enable FP16/BF16 mixed precision to save VRAM
        ...
        cudagraphs=False,
        compile=False,  # disable torch.compile for faster startup in test
    ),
    motions='',
    motions_root='',
    ...
    work_dir='results/test-smoke',
    seed=42,
    online_parallel_envs=32,
    num_env_steps=5000,            # just 5k steps
    update_agent_every=500,        # update every 500 steps
    num_seed_steps=500,            # fewer seed steps
    num_agent_updates=2,           # fewer updates per step
    checkpoint_every_steps=100000, # no checkpoint during test
    log_every_updates=500,
    checkpoint_buffer=True,
    prioritization=False,          # disable for simplicity
    use_trajectory_buffer=True,
    buffer_size=10000,             # tiny buffer for smoke test
    use_wandb=False,
    load_isaac_expert_data=True,
    buffer_device='cpu',
    disable_tqdm=False,           # show progress
    evaluations=[],               # skip evaluations
    eval_every_steps=1000000,
    tags={'test': True},
)

print("=" * 60)
print("BFM-Zero Smoke Test")
