...
def main():
    ...
    z_specs = {"dyn1": (int(rng.choice(dyn_pool)), int(rng.integers(20, 60))),
               "dyn2": (int(rng.choice(dyn_pool)), int(rng.integers(20, 60))),
    ...
    for arm, ck in arms.items():
        ...
        for s_i in range(N_STATES):
            ...
            zspec = eps_run["dyn2"]["tracking_to_dyn1_mean"] - eps_run["dyn1"]["tracking_to_dyn1_mean"]
            per_state.append({"state": s_i, "g_dyn_minus_stand": g, "z_specific_tracking": zspec, "runs": eps_run})
    ...
    out = {
        "_provenance": {"created_at": datetime.now().isoformat(), "probe": "probe_command_response.py",
    ...
    p = Path("results/bfmzero-bdx-full/probe_command_response.json")
    assert not p.exists()
